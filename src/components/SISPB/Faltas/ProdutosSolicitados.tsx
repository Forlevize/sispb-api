...
export default function ProdutosSolicitados() {
  ...
  useEffect(() => {
    ...
    if (faltasSalvas) {
      ...
      const faltasSolicitadas = faltasCarregadas.filter(
        (falta: Falta) => falta.status !== StatusFalta.EM_ABERTO
      );
      setProdutosFiltrados(faltasSolicitadas);
  ...
  useEffect(() => {
    let resultado = faltas.filter(falta => falta.status !== StatusFalta.EM_ABERTO);
  ...
  const handleAtualizarStatus = (falta: Falta, novoStatus: StatusFalta) => {
    const faltasAtualizadas = faltas.map(f => {
      if (f.id === falta.id) {
    ...
    localStorage.setItem("faltas", JSON.stringify(faltasAtualizadas));
  ...
  const statusOptionsFiltradas = Object.values(StatusFalta).filter(
    status => status !== StatusFalta.EM_ABERTO
  );
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold flex items-center">
                <ShoppingCart className="mr-2 h-5 w-5 text-[#00c6a7]" />
            ...
            <FiltrosComponent
              ...
              unidades={unidades}
              statusOptions={statusOptionsFiltradas}
